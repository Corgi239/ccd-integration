functions{
  ...
  matrix simulate(
      ...
      int[] cmt,
      real[] parms,
      data real rtol, data real atol, int max_num_steps
    ){
    int nt = size(time);
    int nCmt = 8;
    matrix[nCmt, nt] x;
    real last_x[nCmt] = rep_array(0., 8);
    real last_t = 0;
    for(i in 1:nt){
      if(time[i] > last_t){
        last_x = integrate_ode_rk45(
          twoCptNeutModelODE, last_x, last_t, {time[i]}, parms,
          rep_array(0., 0), rep_array(0, 0),
...
generated quantities{
  ...
  x = simulate(
      time, amt, evid, cmt, {CL, Q, V1, V2, ka, mtt, circ0, gamma, alpha},
      1e-6, 1e-6, 1e8
    );
